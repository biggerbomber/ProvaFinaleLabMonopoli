...
Bot::Bot(int tag, Position p,std::shared_ptr<Player>* self ,Logger* l) {// problema qui " impossibile fare riferimento al costruttore predefinito player, e' una funzione eliminata"
    m_tag = tag;
    m_posizione = p;
    ...
    if (t->get_tile_type() != Tile::TileType::ANGOLARE) {
        if (!t->has_proprietario()) {
            ...
            if (i == 1) { // 1 numero puramente arbitrario ma con P(1)=1/4
                ...
                m_log->log(Logger::ACQUISTO_TERRENO, m_tag, m_posizione);
            ...
            if (t->get_proprietario()->get_tag() == m_tag) {
                ...
                if (!paga(t->get_costo_pernottamento())) {
                    ...
                    m_log->log(Logger::ELIMINAZIONE, m_tag);
                    return false;
                }
                else {
                     t->get_proprietario()->riscuoti(t->get_costo_pernottamento());
                     m_log->log(Logger::PAGAMENTO_PERNOTTAMENTO, m_tag, m_posizione, t->get_proprietario()->get_tag(), t->get_costo_pernottamento());
                }
            }
        }
    }
